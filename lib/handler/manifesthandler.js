(function() {
    ...
    module.exports.rewrite = function (filename, context) {
        ...
        parser.on('startElement', function (name, attrs) {
            if (name.match(/manifest/i)) {
                currentPackageName = attrs.package
            ...
            if (name.match('activity') || name.match('service') || name.match('receiver') || name.match('provider')) {
                // modify android:name like android:name=".MainActivity"
                if (attrs['android:name'].match(/^\./) && context.oldPackageName == currentPackageName) {
                    modified = true
                    var policy = context.policy.rewriteIncompleteName(context, attrs['android:name'])

                    // replace activity name define
                    context.log.i('modify ' + name + ' name: ' + policy.name + ' => ' + policy.target)
                    replaceHelper.replace(
                                file.lines,
                                bufferLineNumber,
                                currentLineNumber,
